...
import { matchMarketsAcrossPlatforms, searchBasedMatch, } from '../lib/arbitrage/matcher.js';
import { findArbitrageOpportunities, calculateProfit, } from '../lib/arbitrage/engine.js';
...
// Market fetching
// ---------------------------------------------------------------------------
async function fetchKalshiMarkets(client, category) {
...
async function fetchPolymarkets(category) {
    const cfg = CATEGORIES[category] || CATEGORIES.all;
    ...
    const fetchByVolume = async (limit, offset = 0) => {
        try {
...
// Output formatting
// ---------------------------------------------------------------------------
function formatOpportunityTable(opportunities, maxRows) {
...
function formatDetailedOpportunity(opp, idx, stake) {
...
async function scanKalshiEventMispricing(client) {
    const results = [];
    try {
...
async function scanPolymarketEventMispricing() {
    const results = [];
    const tempClient = new PolymarketClient(POLY_DUMMY_KEY);
    try {
        ...
        for (const event of events) {
            ...
            for (const m of event.markets) {
                if (!m.active || m.closed)
                    continue;
                const parsed = PolymarketClient.parseMarketFields(m);
...
// ---------------------------------------------------------------------------
async function domeMatchedPairs(kalshiMarkets, polyMarkets) {
    const results = [];
    try {
        ...
        for (const [, platforms] of Object.entries(matchMap)) {
            ...
            if (!kalshiEntry?.market_tickers?.length || !polyEntry?.market_slug)
                continue;
            const polyMkt = polyBySlug.get(polyEntry.market_slug);
            if (!polyMkt)
                continue;
            const parsed = PolymarketClient.parseMarketFields(polyMkt);
...
export function registerArbitrageTools(server) {
    ...
    server.tool({
        name: 'scan_arbitrage',
        description: 'Scan for cross-platform arbitrage (Kalshi vs Polymarket) and multi-outcome mispricing. ' +
    ...
    }, async ({ category, min_edge, max_results, use_search, example_stake }, ctx) => {
        ...
        try {
            ...
            if (use_search) {
                ...
                const usedPolyIds = new Set(fuzzyMatched.map((m) => polyMarkets.find((p) => p.slug === m.polymarketSlug)?.id).filter(Boolean));
                const polyClient = new PolymarketClient(POLY_DUMMY_KEY);
                searchMatched = await searchBasedMatch(kalshiMarkets, alreadyMatched, polyClient, 0.28, 5, 25, polyMarkets, usedPolyIds);
            }
    ...
    server.tool({
        ...
        description: 'Find events where outcome prices don\'t sum to $1 (single-platform mispricing). ' +
    ...
    }, async ({ platform, min_edge, max_results, example_stake }, ctx) => {
        const state = getSession(getSessionId(ctx));
        const t0 = Date.now();
        const allOpps = [];
        try {
            ...
            if (top.length === 0) {
                return markdown(`## Event Mispricing Scan — No Opportunities\n\n` +
    ...
    server.tool({
        name: 'analyze_edge',
        description: 'Deep-dive edge analysis on a single market, comparing prediction market price vs ESPN sportsbook implied probabilities. ' +
        ...
        if (platform === 'kalshi') {
            ...
            try {
                ...
                const parsed = PolymarketClient.parseMarketFields(market);
    ...
    server.tool({
        name: 'quick_arb',
        description: 'Find the best arbitrage opportunity and execute both legs automatically. ' +
    ...
    }, async ({ category, max_stake, dry_run }, ctx) => {
        const state = getSession(getSessionId(ctx));
        if (!state.kalshi)
            return error('Kalshi authentication required. Run kalshi_login first.');
        const t0 = Date.now();
        try {
            ...
            if (opportunities.length === 0) {
                return text(`No arbitrage found across ${kalshiMarkets.length} Kalshi × ${polyMarkets.length} Polymarket markets ` +
            ...
            if (profit.contracts < 1) {
                return text(`Best opportunity: ${(best.edge * 100).toFixed(2)}% edge, but needs at least ${formatDollars(best.totalCost)} per pair.\n` +
                ...
                if (tokenId) {
                    try {
                        ...
                        const order = await state.polymarket.client.placeOrder({
                            ...
                            size: profit.contracts, side: pmSide, orderType: 'GTC',
                        });
                        orders.push({
