...
import { decimalToKalshiCents, kalshiCentsToDecimal, formatDollars, } from '../lib/utils/normalize.js';
export function registerTradingTools(server) {
    server.tool({
        name: 'place_order',
        description: 'Place a buy or sell order on Kalshi or Polymarket. ' +
        ...
        schema: z.object({
            ...
            market_id: z
                .string()
                .describe('Market identifier: Kalshi ticker or Polymarket token_id (for the specific outcome)'),
            side: z
                .enum(['yes', 'no'])
    ...
    }, async ({ platform, market_id, side, action, quantity, price, order_type }, ctx) => {
        if (platform === 'kalshi') {
            ...
            try {
                ...
                const orderInput = {
                    ...
                    action,
                    side,
                    type: order_type === 'market' ? 'limit' : order_type,
                    count: quantity,
                    ...(side === 'yes'
                ...
                return error(`Kalshi order failed: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
        else {
            const state = getSession(getSessionId(ctx));
            ...
            try {
                ...
                return error(`Polymarket order failed: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
    });
    server.tool({
        name: 'cancel_order',
        description: 'Cancel an open order. WHEN: User wants to cancel, or an order is unfilled and conditions have changed. ' +
        ...
        if (platform === 'kalshi') {
            ...
            try {
                ...
                return error(`Failed to cancel Kalshi order: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
        else {
            if (!state.polymarket) {
            ...
            try {
                ...
                return error(`Failed to cancel Polymarket order: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
    });
}
