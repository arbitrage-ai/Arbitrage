 ...
 */
function computeScore(kalshiText, polyText) {
    ...
    if (kAbbr.length > 0 && pAbbr.length > 0) {
        ...
        let hits = 0;
        for (const a of kAbbr)
            if (pSet.has(a))
                hits++;
        abbrMatch = hits / Math.max(kAbbr.length, pAbbr.length);
    }
    // Strong entity matches → high confidence shortcut
    if (numMatch > 0.5 && (nounMatch > 0.3 || kwOverlap > 0.4))
        return 0.85 + stringSim * 0.1;
    if (nounMatch > 0.6 && kwOverlap > 0.3)
        return 0.75 + stringSim * 0.15;
    // Weighted combination
    return (stringSim * 0.25 +
        kwOverlap * 0.30 +
        numMatch * 0.20 +
        nounMatch * 0.15 +
        abbrMatch * 0.10);
}
function parsePolyPrices(pm) {
    const parsed = PolymarketClient.parseMarketFields(pm);
    return {
 ...
 */
export function matchMarketsAcrossPlatforms(kalshiMarkets, polyMarkets, threshold = 0.35) {
    const results = [];
    ...
    for (const km of kalshiMarkets) {
        ...
        for (let i = 0; i < polyMarkets.length; i++) {
            if (usedPolyIds.has(polyMarkets[i].id))
                continue;
 ...
 */
function matchOneAgainstPoly(km, polyMarkets, threshold) {
    const kmText = `${km.title} ${km.subtitle}`;
    let bestScore = 0;
    let bestPm = null;
    for (const pm of polyMarkets) {
        if (!pm.question)
            continue;
        if (pm.closed === true || pm.active === false)
            continue;
        const polyText = `${pm.eventTitle || ''} ${pm.question}`.trim();
 ...
 */
export async function searchBasedMatch(kalshiMarkets, alreadyMatchedTickers, polyClient, threshold = 0.30, batchSize = 5, maxSearches = 25, polyMarketsFallback, usedPolyIds) {
    const results = [];
    ...
    for (let i = 0; i < toSearch.length; i += batchSize) {
        ...
        const promises = batch.map(async (km) => {
                    ...
                    if (Array.isArray(searchResults) && searchResults.length > 0) {
                        ...
                        for (const item of searchResults) {
                            ...
                            if (Array.isArray(obj.markets)) {
                                polyMarkets.push(...obj.markets);
                            }
                            if (typeof obj.question === 'string') {
                                polyMarkets.push(obj);
                            }
                        }
