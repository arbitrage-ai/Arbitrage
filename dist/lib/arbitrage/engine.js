 ...
 */
export function findArbitrageOpportunities(matched, minEdge = 0.005) {
    const opportunities = [];
    for (const m of matched) {
        if (m.kalshiYesPrice <= 0 || m.kalshiNoPrice <= 0)
            continue;
        if (m.polymarketYesPrice <= 0 || m.polymarketNoPrice <= 0)
            continue;
...
export function calculateProfit(opp, maxStake) {
