...
import { clearKalshiSession, clearPolymarketSession, getSession, setKalshiSession, setPolymarketSession, } from '../lib/utils/session.js';
import { getSessionId } from '../lib/utils/ctx.js';
import { formatDollars } from '../lib/utils/normalize.js';
async function resolveKalshiPrivateKey(input) {
...
export function registerAuthTools(server) {
    server.tool({
        name: 'kalshi_login',
        description: 'Authenticate with Kalshi. Required before scan_arbitrage, quick_arb, place_order (Kalshi), or any Kalshi-specific data. ' +
        ...
        schema: z.object({
            ...
            private_key_pem: z
                .string()
                .describe('Your RSA private key in PEM format (starts with -----BEGIN RSA PRIVATE KEY-----)'),
        }),
    }, async ({ api_key_id, private_key_pem }, ctx) => {
        ...
        try {
            ...
            return text(`Kalshi authenticated successfully.\nBalance: ${formatDollars(balance.balance / 100)}`);
        }
        catch (e) {
            ...
            if (msg.includes('DECODER routines::unsupported')) {
                return error('Kalshi authentication failed: invalid private key format. Provide PEM content (including BEGIN/END lines) or a valid path to the PEM file.');
            }
            return error(`Kalshi authentication failed: ${msg}`);
        }
    });
    server.tool({
        ...
        description: 'Authenticate with Polymarket. Required before place_order (Polymarket), quick_arb execution, or fetching Polymarket positions/balance. ' +
        ...
        schema: z.object({
            private_key: z
                .string()
                .describe('Your Ethereum wallet private key (hex, with or without 0x prefix)'),
            api_key: z
                .string()
            ...
            funder_address: z
                .string()
                .optional()
                .describe('Your Polymarket proxy wallet address (from polymarket.com/settings). Required if using a proxy wallet.'),
        }),
    }, async ({ private_key, api_key, secret, passphrase, funder_address }, ctx) => {
        const sessionId = getSessionId(ctx);
        try {
            ...
            setPolymarketSession(sessionId, normalizedKey, {
                apiKey: api_key,
                secret,
                passphrase,
            }, funder_address);
            return text(`Polymarket credentials saved successfully.\nWallet: ${client.address}\n` +
    ...
    server.tool({
        name: 'auth_status',
        description: 'Check authentication status and balances for all platforms. ' +
