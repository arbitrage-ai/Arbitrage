...
function normalizePrivateKeyPem(privateKeyPem) {
    ...
    if (!beginMatch && endMatch) {
        throw new Error('Private key appears malformed: missing -----BEGIN ... PRIVATE KEY----- marker.');
    }
    if (beginMatch && !endMatch) {
    ...
    if (beginMatch && endMatch) {
        ...
        if (label !== endLabel) {
            throw new Error(`Private key markers mismatch: BEGIN ${label} but END ${endLabel}.`);
        }
        const beginMarker = `-----BEGIN ${label}-----`;
...
export class KalshiClient {
    apiKeyId;
    privateKey;
    constructor(apiKeyId, privateKeyPem) {
    ...
    async request(method, path, body) {
        ...
        if (!response.ok &&
            ...
            const isBadSignature = errorText.includes('INCORRECT_API_KEY_SIGNATURE') ||
    ...
    async getPositions(params) {
        ...
        if (params?.status)
            searchParams.set('settlement_status', params.status);
        if (params?.limit)
